...
$url = "https://raw.githubusercontent.com/$repo/$ref/shell/powershell/install-remote.ps1?v=$cacheBuster"
...
try {
    Invoke-WebRequest -Uri $url -OutFile $tempScript -UseBasicParsing -Headers @{"Cache-Control"="no-cache"}
    & $tempScript @args
} finally {
