...
| `cc-openai` | 18419 | **openai**: `gpt-5.3-openai` 계열의 high/xhigh/spark 모델로 매핑 |
...
*   **교차 프로바이더 재개**: 서로 다른 포트를 쓰는 프로바이더(예: `cc-claude` → `cc-openai`) 간에도 기본적으로는 재개가 가능합니다.
...
4.  **팀 리더의 선택**: 대규모 병렬 작업(에이전트 다수 호출) 시에는 Rate Limit이 넉넉한 `cc-openai` 프로바이더를 리더로 사용하는 것이 가장 유리합니다.
